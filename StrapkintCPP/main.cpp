...
int main() {
	...
	constexpr double horizontalSpeed = 10;
	...
	const auto adjustWriteRegion = [&writeRegion, &bufferPosition](short x, short y, short width, short height = 1) {
		const short right = x + width - 1;
		const short bottom = y + height - 1;
	...
	while (true) {
...

		{
			...
			for (const auto key : pressedKeys) {
				...
				case VK_LEFT:
				{
					const short before = text.x;
					collision.x -= dtSeconds * horizontalSpeed;
					if (collision.x < 0)
						collision.x = 0;
					const short after = (short)round(collision.x);
					const short difference = before - after;
					if (difference) {
				...
				case VK_RIGHT:
				{
					const short before = text.x;
					collision.x += dtSeconds * horizontalSpeed;
					if (collision.x > level.width - collision.width)
		...
		if (eventsRead) {
			...
			for (int i = 0; i < eventsRead; ++i) {
				if (inputRecords[i].EventType == KEY_EVENT) {
					...
					if (event.bKeyDown) {
						if (!pressedKeys.contains(event.wVirtualKeyCode)) {
